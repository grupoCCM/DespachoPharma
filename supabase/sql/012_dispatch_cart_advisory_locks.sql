...
begin
  ...
  create temporary table if not exists pg_temp.dispatch_cart_items(
    barcode text primary key,
    qty integer not null
  ) on commit drop;

  truncate table pg_temp.dispatch_cart_items;

  insert into pg_temp.dispatch_cart_items(barcode, qty)
  select barcode, qty
  from (
    select
  ...
  from pg_temp.dispatch_cart_items;

  if coalesce(v_count, 0) = 0 then
    raise exception 'Carrito vacio';
  end if;

  -- Bloquea solo los productos del carrito durante esta transaccion.
  -- El orden estable evita deadlocks si dos usuarios despachan varios productos iguales.
  for v_item in
    select barcode
    from pg_temp.dispatch_cart_items
    order by barcode
  loop
    perform pg_advisory_xact_lock(hashtextextended('dispatch_stock:' || v_item.barcode, 0));
  end loop;
  ...
  insert into public.dispatch_header(expediente, status, created_by)
  values (trim(p_expediente), 'draft', v_user_id)
  ...
  values (
    ...
    v_user_id,
    v_dispatch_id,
    jsonb_build_object('delivery_no', v_delivery, 'items', v_count, 'mode', 'submit_cart', 'stock_lock', 'advisory_xact')
  );
