...
begin
  ...
  base as (
    select
      ...
      pr.action as recommendation_action,
      pr.expiration_risk,
      lp.last_cost,
      pc.avg_cost,
      pc.min_cost,
  ...
  adjusted as (
    select
      *,
      case
        when stock_qty > 0 and coalesce(recommendation_action, '') ilike 'Pedir ya:%sin existencia%'
          then 'Vigilar: existe inventario actual'
        else recommendation_action
      end as action,
      case
        when stock_qty <= 0 and coalesce(recommendation_action, '') ilike 'Pedir ya%' then 'Pedir'
        when stock_qty <= 0 and coalesce(monthly_rotation, 0) > 0 then 'Pedir'
        when coalesce(suggested_qty, 0) > 0 then 'Vigilar'
        else 'Esperar'
      end as decision
    from base
  )
