...
declare
  ...
  v_sales_profit_checkpoint jsonb;
begin
  ...
  select to_jsonb(x) into v_sales_profit_checkpoint
  from (
    with last_date as (
    ...
    select
      ...
      round(coalesce(sum(si.net_sale), 0)::numeric, 2) as net_sales,
      round(coalesce(sum(si.profit), 0)::numeric, 2) as profit,
      (array_agg(sd.source_file order by sd.created_at desc nulls last) filter (where sd.source_file is not null))[1] as source_file,
      max(sd.created_at) as created_at
    from last_date ld
    join public.sales_documents sd on sd.sale_date = ld.sale_date
    left join public.sales_items si on si.sales_document_id = sd.id
    where ld.sale_date is not null
    group by ld.sale_date
  ) x;
  ...
  if v_batch_key is null then
    return jsonb_build_object(
      ...
      'sales_profit_checkpoint', v_sales_profit_checkpoint
    );
  end if;
