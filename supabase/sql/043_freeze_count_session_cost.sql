alter table public.inventory_count_session_items
  add column if not exists expected_unit_cost numeric(14,6);

update public.inventory_count_session_items i
set expected_unit_cost = coalesce(i.expected_unit_cost, vl.unit_cost, 0)
from public.vw_inventory_live vl
where vl.external_code = i.external_code
  and i.expected_unit_cost is null;

update public.inventory_count_session_items
set expected_unit_cost = 0
where expected_unit_cost is null;
...
begin
  ...
  values ('INVENTORY_COUNT_SESSION_CREATE', v_user_id, jsonb_build_object('session_id', v_session_id, 'scope', v_scope, 'frozen_fields', jsonb_build_array('expected_qty','expected_unit_cost')));
...
begin
  ...
  with base as (
    select
      ...
      i.counted_qty,
      i.variance_qty,
      coalesce(i.expected_unit_cost, 0) as unit_cost,
      coalesce(i.variance_qty, 0) * coalesce(i.expected_unit_cost, 0) as variance_value,
      i.status,
      i.note,
