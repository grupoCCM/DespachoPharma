-- Allow operational users to finish or cancel physical count sessions.
-- Applying inventory differences remains admin-only in rpc_inventory_count_session_item_apply.
...
begin
  ...
  if v_role not in ('admin','dispatch','cashier') then
    raise exception 'No autorizado para finalizar conteos';
  end if;
