create or replace view public.vw_inventory_sellable as
with expired_lots as (
  select
    er.external_code,
    coalesce(sum(greatest(coalesce(er.lot_qty, 0), 0)) filter (where er.expiration_status = 'expired'), 0)::numeric as expired_qty,
    min(er.expires_at) filter (where er.lot_qty > 0) as nearest_expiration
  from public.vw_expiration_risk_latest er
  where er.external_code is not null
  group by er.external_code
)
select
  live.external_code,
  live.medicine_id,
  live.barcode,
  live.description_snapshot,
  live.model,
  live.presentation,
  greatest(coalesce(live.stock_qty, 0), 0)::numeric as physical_qty,
  coalesce(expired.expired_qty, 0)::numeric as expired_qty,
  0::numeric as not_sellable_qty,
  greatest(coalesce(live.stock_qty, 0) - coalesce(expired.expired_qty, 0), 0)::numeric as sellable_qty,
  live.unit_cost,
  live.stock_value,
  live.snapshot_created_at,
  expired.nearest_expiration
from public.vw_inventory_live live
left join expired_lots expired on expired.external_code = live.external_code;

grant select on public.vw_inventory_sellable to anon, authenticated;

create or replace function public.rpc_public_availability_search(p_query text, p_limit integer default 30)
returns jsonb
language sql
security definer
set search_path = public
as $$
with latest_ref as (
  select distinct on (external_code)
    external_code,
    reference_price
  from public.medicine_reference_prices
  order by external_code, loaded_at desc
),
latest_rotation as (
  select rr.id
  from public.rotation_runs rr
  order by rr.cutoff_date desc nulls last, rr.created_at desc
  limit 1
),
base as (
  select
    m.external_code,
    m.name as medicine_name,
    m.secondary_name,
    m.brand_name,
    m.presentation_name,
    m.model,
    m.subgroup_name,
    m.barcode,
    m.active,
    m.price_1 as sale_price,
    coalesce(i.sellable_qty, 0)::numeric as stock_qty,
    coalesce(i.physical_qty, 0)::numeric as physical_qty,
    coalesce(i.expired_qty, 0)::numeric as expired_qty,
    coalesce(i.not_sellable_qty, 0)::numeric as not_sellable_qty,
    coalesce(i.stock_value, 0) as stock_value,
    coalesce(o.reference_price, lr.reference_price) as reference_price,
    rm.monthly_rotation,
    rm.coverage_months,
    rm.last_sale_date
  from public.medicines m
  left join public.vw_inventory_sellable i on i.external_code = m.external_code
  left join latest_ref lr on lr.external_code = m.external_code
  left join public.medicine_reference_price_overrides o on o.external_code = m.external_code
  left join latest_rotation r on true
  left join public.rotation_metrics rm on rm.rotation_run_id = r.id and rm.external_code = m.external_code
  where m.active is true
    and coalesce(i.sellable_qty, 0) > 0
    and (
      coalesce(trim(p_query), '') = ''
      or m.external_code::text = trim(p_query)
      or coalesce(m.barcode, '') ilike '%' || trim(p_query) || '%'
      or m.name ilike '%' || trim(p_query) || '%'
      or coalesce(m.secondary_name, '') ilike '%' || trim(p_query) || '%'
      or coalesce(m.model, '') ilike '%' || trim(p_query) || '%'
      or coalesce(m.brand_name, '') ilike '%' || trim(p_query) || '%'
      or coalesce(m.presentation_name, '') ilike '%' || trim(p_query) || '%'
      or coalesce(m.subgroup_name, '') ilike '%' || trim(p_query) || '%'
    )
)
select coalesce(jsonb_agg(to_jsonb(x)), '[]'::jsonb)
from (
  select
    external_code,
    medicine_name,
    secondary_name,
    brand_name,
    presentation_name,
    model,
    subgroup_name,
    barcode,
    active,
    stock_qty,
    physical_qty,
    expired_qty,
    not_sellable_qty,
    sale_price,
    reference_price,
    monthly_rotation,
    coverage_months,
    last_sale_date,
    case
      when stock_qty < 15 then 'Pocas unidades'
      else 'Disponible'
    end as availability_status
  from base
  order by
    case when stock_qty < 15 then 1 else 0 end,
    medicine_name
  limit greatest(1, least(coalesce(p_limit, 30), 100))
) x;
$$;

grant execute on function public.rpc_public_availability_search(text, integer) to anon, authenticated;

create or replace function public.rpc_public_availability_suggest(p_query text, p_limit integer default 12)
returns jsonb
language sql
security definer
set search_path = public
as $$
with query_input as (
  select lower(trim(coalesce(p_query, ''))) as q
),
latest_ref as (
  select distinct on (external_code)
    external_code,
    reference_price
  from public.medicine_reference_prices
  order by external_code, loaded_at desc
),
latest_rotation as (
  select rr.id
  from public.rotation_runs rr
  order by rr.cutoff_date desc nulls last, rr.created_at desc
  limit 1
),
base as (
  select
    m.external_code,
    m.name as medicine_name,
    m.secondary_name,
    m.brand_name,
    m.presentation_name,
    m.model,
    m.subgroup_name,
    m.barcode,
    m.active,
    m.price_1 as sale_price,
    coalesce(i.sellable_qty, 0)::numeric as stock_qty,
    coalesce(i.physical_qty, 0)::numeric as physical_qty,
    coalesce(i.expired_qty, 0)::numeric as expired_qty,
    coalesce(i.not_sellable_qty, 0)::numeric as not_sellable_qty,
    coalesce(i.stock_value, 0) as stock_value,
    coalesce(o.reference_price, lr.reference_price) as reference_price,
    rm.monthly_rotation,
    rm.coverage_months,
    rm.last_sale_date,
    case
      when qi.q = '' then 10
      when m.external_code::text = qi.q or lower(coalesce(m.barcode, '')) = qi.q then 0
      when lower(m.name) like qi.q || '%' then 1
      when lower(m.name) like '%' || qi.q || '%' then 2
      when lower(coalesce(m.secondary_name, '')) like qi.q || '%' then 3
      when lower(coalesce(m.secondary_name, '')) like '%' || qi.q || '%' then 4
      when lower(coalesce(m.model, '')) like qi.q || '%' then 5
      when lower(coalesce(m.model, '')) like '%' || qi.q || '%' then 6
      else 7
    end as search_rank
  from public.medicines m
  cross join query_input qi
  left join public.vw_inventory_sellable i on i.external_code = m.external_code
  left join latest_ref lr on lr.external_code = m.external_code
  left join public.medicine_reference_price_overrides o on o.external_code = m.external_code
  left join latest_rotation r on true
  left join public.rotation_metrics rm on rm.rotation_run_id = r.id and rm.external_code = m.external_code
  where m.active is true
    and qi.q <> ''
    and (
      m.external_code::text = qi.q
      or lower(coalesce(m.barcode, '')) like '%' || qi.q || '%'
      or lower(m.name) like '%' || qi.q || '%'
      or lower(coalesce(m.secondary_name, '')) like '%' || qi.q || '%'
      or lower(coalesce(m.model, '')) like '%' || qi.q || '%'
      or lower(coalesce(m.brand_name, '')) like '%' || qi.q || '%'
      or lower(coalesce(m.presentation_name, '')) like '%' || qi.q || '%'
      or lower(coalesce(m.subgroup_name, '')) like '%' || qi.q || '%'
    )
)
select coalesce(jsonb_agg(to_jsonb(x)), '[]'::jsonb)
from (
  select
    external_code,
    medicine_name,
    secondary_name,
    brand_name,
    presentation_name,
    model,
    subgroup_name,
    barcode,
    active,
    stock_qty,
    physical_qty,
    expired_qty,
    not_sellable_qty,
    sale_price,
    reference_price,
    monthly_rotation,
    coverage_months,
    last_sale_date,
    case
      when stock_qty <= 0 then 'Sin existencia'
      when stock_qty < 15 then 'Pocas unidades'
      else 'Disponible'
    end as availability_status
  from base
  order by
    search_rank,
    case when stock_qty <= 0 then 1 else 0 end,
    medicine_name
  limit greatest(1, least(coalesce(p_limit, 12), 30))
) x;
$$;

grant execute on function public.rpc_public_availability_suggest(text, integer) to anon, authenticated;

create or replace function public.dispatch_stock_check(
  p_barcode text,
  p_requested_qty numeric,
  p_exclude_dispatch_id uuid default null
)
returns table(
  external_code integer,
  medicine_name text,
  stock_qty numeric,
  reserved_qty numeric,
  available_qty numeric,
  requested_qty numeric,
  ok boolean
)
language sql
security definer
set search_path = public
as $$
with matched as (
  select apm.barcode, apm.product_name
  from public.app_pharma_match(p_barcode) apm
  where apm.active is true
  limit 1
),
med as (
  select m.external_code, coalesce(m.name, matched.product_name) as medicine_name
  from matched
  join public.medicines m on m.barcode = matched.barcode
  order by m.active desc, m.external_code
  limit 1
),
sellable_stock as (
  select coalesce(vs.sellable_qty, 0)::numeric as qty
  from med
  left join public.vw_inventory_sellable vs on vs.external_code = med.external_code
)
select
  med.external_code,
  med.medicine_name,
  sellable_stock.qty as stock_qty,
  0::numeric as reserved_qty,
  greatest(sellable_stock.qty, 0) as available_qty,
  coalesce(p_requested_qty, 0)::numeric as requested_qty,
  med.external_code is not null
    and coalesce(p_requested_qty, 0) > 0
    and greatest(sellable_stock.qty, 0) >= coalesce(p_requested_qty, 0)::numeric as ok
from med
cross join sellable_stock;
$$;

grant execute on function public.dispatch_stock_check(text, numeric, uuid) to anon, authenticated;

notify pgrst, 'reload schema';
