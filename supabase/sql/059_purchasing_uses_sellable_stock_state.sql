-- Purchasing must evaluate reorder decisions against sellable stock, not raw
-- physical stock. Keep the legacy JSON field stock_qty for UI compatibility,
-- but source it from vw_product_stock_state.sellable_qty.
...
begin
  ...
  expiration as (
    select distinct on (ls.external_code)
      ls.external_code, ls.expires_at as nearest_expiration, ls.days_to_expire, ls.expiration_status
    from public.vw_lot_stock_state ls
    where ls.physical_lot_qty > 0
    order by ls.external_code, ls.expires_at nulls last, ls.days_to_expire nulls last
  ),
  base as (
    select
      m.external_code,
      ...
      coalesce(i.reserved_qty, 0)::numeric as reserved_qty,
      coalesce(i.quarantine_qty, 0)::numeric as quarantine_qty,
      coalesce(i.not_sellable_qty, 0)::numeric as not_sellable_qty,
      0::numeric as inbound_order_qty,
      coalesce(i.physical_stock_value, 0) as stock_value,
      ...
      case when coalesce(sr.avg_daily_sales, 0) > 0 then round(coalesce(i.sellable_qty, 0)::numeric / sr.avg_daily_sales, 1) else null end as coverage_days,
    ...
    left join public.vw_product_stock_state i on i.external_code = m.external_code
  ...
  adjusted as (
    select
      *,
      case
        when purchase_blocked then 0::numeric
        else greatest(ceil(target_stock_qty - (stock_qty + inbound_order_qty)), 0)::numeric
